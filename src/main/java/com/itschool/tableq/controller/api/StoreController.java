...
public class StoreController {
    ...
    @GetMapping("/{storeid}")
    public ResponseEntity<StoreResponse> findStoreById(@PathVariable("storeid") String storeid) {
        Store store = storeService.findById(storeid);
        return ResponseEntity.ok(new StoreResponse(store));
    }

    @PutMapping("{storeid}")
    public ResponseEntity<StoreResponse> update(@PathVariable String storeid, @RequestBody String body){//수정허용은 어디까지?
    ...
    public ResponseEntity<StoreResponse> delete(@PathVariable String storeid){
